...
|---|---|---|
| `20260803000006_two_of_three_answers.sql` | Unreleased | `verify_recovery_answers` now passes on 2 of 3 correct answers instead of 3 of 3. All three are still asked and no per-question result is returned — see [ADR 0012](decisions/0012-security-questions-as-interim-recovery.md) for why both of those matter. |
...
| `20260803000003_intake_images.sql` | Unreleased | ✅ | Private `intake` storage bucket (5 MB cap, images-only MIME allowlist) and four `storage.objects` policies scoping every read and write to `<user_id>/`. No public-read policy — the app uses signed URLs. |
...
All migrations are applied in order against stock Postgres 16 with the RLS
harness passing (36 assertions) before anything is committed. The harness stubs
just enough of `storage` — `buckets`, `objects` and `foldername()` — to check
the bucket config and the folder isolation.
